...
async def main():
    ...
    log_message("Server", f"Loaded server configuration: {config_args.server_host}:{config_args.server_port }")
    try:
        server = WebSocketServerAPI(host=config_args.server_host, port=config_args.server_port, config_args=config_args)
        await server.start()
    except Exception as e:
        print(f"Error starting server: {e}")
        log_message("Server", f"Error starting server: {e}", level='error')
